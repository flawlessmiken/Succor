...
class SubjectViewModelFactory(
    private val dataSourceQuestionsDbDao: QuestionsDbDao,
    private val dataSourceTopicsDbDao:  TopicsDbDao,
    private val dataSourceLessonItemDbDao: LessonItemDbDao,
    private val application: Application ) : ViewModelProvider.Factory {
    @Suppress("unchecked_cast")
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(SubjectViewModel::class.java)) {
            return SubjectViewModel(dataSourceQuestionsDbDao,dataSourceTopicsDbDao,dataSourceLessonItemDbDao,application) as T
        }
